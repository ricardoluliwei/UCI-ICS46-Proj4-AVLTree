...
private:
   ...
   bool contains(Node* root, const Key &k) const;
   bool find(Node* root, const Key & k, Value & answer);
public:
	MyAVLTree();
...
template<typename Key, typename Value>
bool MyAVLTree<Key, Value>::contains(Node* root, const Key &k) const
{
   if (!root)
      return false;
   ...
   return contains(root->leftChild, k) || contains(root->rightChild, k);
}

template<typename Key, typename Value>
bool MyAVLTree<Key, Value>::contains(const Key &k) const
{
   return contains(mRoot, k);
}

template<typename Key, typename Value>
bool MyAVLTree<Key, Value>::find(Node* root, const Key & k, Value & answer)
{
   if (!root)
      return false;
   ...
   return find(root->leftChild, k, answer) || find(root->rightChild, k, answer);
}
